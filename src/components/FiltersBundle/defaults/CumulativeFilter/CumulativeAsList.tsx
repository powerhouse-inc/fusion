...
const CumulativeAsList: FC<CustomCheckboxProps> = ({ filter }) => {
    ...
    >
      <Container>
        {filter.options?.map((option, index) => {
          const isActive = filter.selected && filter.cumulativeType === option.value;
            ...
            <Fragment key={`${option.value}-${index}`}>
              {filter.customOptionsRender !== undefined ? (
                ...
                <DivWithRadioOption
                  onClick={() => {
                    filter.selected &&
                      filter.handleToggleCumulative !== undefined &&
                      filter.handleChangeCumulativeType(option.value as 'absolute' | 'relative');
                  }}
                  isActive={isActive}
                >
                  <OptionContent>
...
export default CumulativeAsList;
